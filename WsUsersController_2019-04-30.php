...
{   
    ...
    public function getEmailRegister(){
        ...
        if ($people) {
            ...
            print json_encode(['noExist'=> true, 'msg' => '¡Lo sentimos!. La identificación no registra en el sistema, por favor acércate a la Fundación Alejandro Londoño. ']);
            exit();
        }
    ...
    public function getOrdersByRol(){
        ...
        if ($rol == "1") {//[ENTIDAD]
            ...
            INNER JOIN orders Orders ON (Orders.id = OrderAppointments.orders_id and Orders.centers_id = 1)
    ...
    // Se almacena registro de control de impresiones al igual que se hace desde fundacion
    public function addPrintControl(){
        $data = $this->request->input('json_decode');
        $this->loadModel('PrintControls');
        $this->loadModel('DeliveryResults');
        ...
        $this->PrintControls->save($printControl);
        
        $dataResults = [
            'attentions_id' => $data->attention_id,
            'results_id' => $data->results_id,
            'users_id' => (int) $data->users_id,
            'delivered' => $data->people,
            'observations' => "Impreso desde aplicación de resultados",
            'phone' => "",

        ];

        $deliveryResult = $this->DeliveryResults->newEntity();

        $deliveryResult = $this->DeliveryResults->patchEntity($deliveryResult, $dataResults);
      
        if ($this->DeliveryResults->save($deliveryResult)) {
            $success = true;
        } else {
            $success = false;
        }
        ...
        print json_encode(['success'=> $success, 'errors' => $errors]);
        exit();
